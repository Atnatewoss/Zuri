...
export function JudgeOnboardingModal() {
  ...
  const handleSetup = async () => {
    ...
    try {
      ...
      if (verifyResult.ok && verifyResult.mockup_url) {
         window.open(verifyResult.mockup_url, '_blank', 'noopener,noreferrer')
      } else {
         // Fallback if the verify endpoint fails
         const fallbackUrl = currentHost.includes('localhost') ? 'http://localhost:5173' : 'https://kuriftu-demo-site.onrender.com'
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogContent className="sm:max-w-[600px] p-0 overflow-hidden border-none shadow-2xl bg-zinc-950 text-white rounded-3xl">
         {/* Top Banner Area */}
         <div className="bg-gradient-to-br from-amber-500/20 via-primary/10 to-transparent p-8 pb-6 border-b border-white/5 relative">
            ...
            <div className="flex items-center gap-3 mb-4">
               ...
               <span className="text-xs font-mono font-medium tracking-widest text-amber-500 uppercase">ALX Hackathon Evaluation</span>
            </div>
            
            <DialogTitle className="text-3xl font-medium tracking-tight mb-2 text-white">
               Welcome to Zuri Command Center
            </DialogTitle>
            <DialogDescription className="text-zinc-400 text-base leading-relaxed">
               Zuri is more than a chatbot. It is an <strong className="text-white font-medium">Action-Capable Guest Assistant</strong> that securely accesses resort data to autonomously resolve guest requests.
            </DialogDescription>
         </div>
         ...
         <div className="p-8 space-y-6">
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
               <h3 className="text-lg font-medium text-white mb-3">One-Click Demo Initialization</h3>
               ...
               <ul className="space-y-3 mb-6">
                  <li className="flex items-start gap-3">
                     <span className="flex-shrink-0 w-6 h-6 rounded-full bg-emerald-500/10 text-emerald-500 flex items-center justify-center text-xs mt-0.5 font-bold">1</span>
                     <span className="text-zinc-300 text-sm">Authorize the demo resort's origin domain dynamically.</span>
                  </li>
                  <li className="flex items-start gap-3">
                     <span className="flex-shrink-0 w-6 h-6 rounded-full bg-emerald-500/10 text-emerald-500 flex items-center justify-center text-xs mt-0.5 font-bold">2</span>
                     <span className="text-zinc-300 text-sm">Launch the live Kuriftu Mockup Site in a new tab.</span>
                  </li>
               </ul>
               ...
               <Button 
                  ...
                  className="w-full h-14 bg-white hover:bg-zinc-200 text-black rounded-xl font-semibold text-base transition-all shadow-[0_0_40px_-10px_rgba(255,255,255,0.3)] hover:shadow-[0_0_60px_-15px_rgba(255,255,255,0.4)]"
               >
                  {isProcessing ? (
                     <div className="flex items-center gap-2">
                        <Loader2 className="w-5 h-5 animate-spin text-zinc-500" />
                        <span className="text-zinc-600">Configuring Access Control...</span>
                     </div>
                  ) : (
                     <div className="flex items-center gap-2">
